from make_size_position import EXAMPLES, SIZE_GRID, XY_GRID
import pandas as pd
import os
from PIL import Image
import numpy as np

records = pd.read_csv('size_position/results.csv').to_dict('records')
records = {
    r['fname']: r for r in records
}
...
class RGBTransform(object):
    ...
    def applied_to(self, image):
        ...
        # Image.convert wants the matrix as a flattened 12-tuple.
        ...
        if channel_count < 3:
            ...
            return Image.merge(''.join(channel_names), channels)
    ...
    def applied_to_pixel(self, color):
        ...
        image1 and image2 should be identical:
            ...
            white = (255, 255, 255)
            size = (100, 100)
            image1 = rgbt.applied_to(Image.new("RGB", size, white))
            image2 = Image.new("RGB", size, rgbt.applied_to_pixel(white))
...
def tint(img, color):
    return RGBTransform().mix_with(color, factor=.30).applied_to(img)
...
cmp_dir = 'size_position/composite'
os.makedirs(cmp_dir, exist_ok=True)
for orig_fname, *_ in EXAMPLES:
    orig_basename = os.path.splitext(orig_fname)[0]
    for size in SIZE_GRID:
        size_fname = os.path.join(cmp_dir, f'composite_{orig_basename}_{size}.jpg')

        comp = np.zeros((224 * len(XY_GRID), 224 * len(XY_GRID), 3), dtype=np.uint8)
        # Loop through and get composites
        for i, x_pos in enumerate(XY_GRID):
            for j, y_pos in enumerate(XY_GRID):
                new_fname = f'{orig_basename}_{x_pos}_{y_pos}_{size}.jpg'
                full_fname = os.path.join('size_position', new_fname)
                img = Image.open(full_fname)
                assert new_fname in records

                if records[new_fname]['success']:
                    img = tint(img, GREEN)
                elif records[new_fname]['partial_success']:
                    img = tint(img, YELLOW)
                else:
                    img = tint(img, RED)

                x_slice = slice(i * 224, (i * 224) + 224)
                y_slice = slice(j * 224, (j * 224) + 224)
                comp[y_slice, x_slice] = np.array(img)

        Image.fromarray(comp).save(size_fname)
