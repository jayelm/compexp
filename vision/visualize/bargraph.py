...
default_category_order = ["object", "scene", "part", "material", "texture", "color"]
...
def bar_graph_svg(
    ...
    for record in tally_result:
        ...
        if label not in label_score or label_score[label] < float(record["score"]):
    ...
    sorted_labels = sorted(
        label_counts.keys(), key=lambda x: (-label_counts[x], -label_score[x])
    )
    ...
    for cat in category_order:
        filtered = [label for label in sorted_labels if label_cats[label] == cat]
    ...
    if save:
        if save == True:
            ...
            filename = save
        ed.ensure_dir("html")
    return make_svg_bargraph(
        labels, heights, categories, barheight, barwidth, show_labels, filename
    )


def make_svg_bargraph(
    labels,
    heights,
    categories,
    barheight=100,
    ...
    for h in [1, (max(heights) + 1) // 2, max(heights)]:
        et.SubElement(
            ...
            % (textsize, x - gap, basey - h * unitheight),
        ).text = str(h)
    et.SubElement(
        svg,
        "text",
    ...
    for label, height in zip(labels, heights):
        ...
        et.SubElement(
            ...
            "rect",
            x=str(x),
            y=str(basey - (height * unitheight)),
            width=str(barwidth),
            height=str(height * unitheight),
            fill=color,
        )
        x += barwidth
        if show_labels:
    ...
    for cat, catcount in categories:
        ...
        et.SubElement(
            ...
            % (
                textsize,
                x + (barwidth + gap) * catcount - gap,
                basey - rollup * unitheight + gap,
            ),
        ).text = "%d %s" % (catcount, fix(cat + ("s" if catcount != 1 else "")))
        x += (barwidth + gap) * catcount
...
if __name__ == "__main__":
    ...
    parser.add_argument("--directory", help="directory to graph")
    parser.add_argument("--blob", nargs="*", help="blob to graph")
    parser.add_argument(
        "--barheight", type=int, default=100, help="graph big color bar height"
    )
    parser.add_argument(
        "--barwidth", type=int, default=12, help="graph little color bar width"
    )
    args = parser.parse_args()

    bar_graph_svg(args.directory, args.blob, args.barheight, args.barwidth, save=True)
