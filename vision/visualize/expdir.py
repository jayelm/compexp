...
class ExperimentDirectory:
    ...
    def glob_number(self, name, last=True, decimal=False, blob=None, part=None):
        ...
        """
        n, fn = self.filename(
            name, last=last, decimal=decimal, blob=blob, part=part, aspair=True
        )
        return n
    ...
    def html_filename(self, blob=None, part=None, **kwargs):
        ...
        result = self.filename(
            "html/%s.html" % ("-".join(filter(None, [fn_safe(blob), part])),), **kwargs
        )
        return result
...
def numbered_glob(pattern, last=True, decimal=False, every=False):
    ...
    repat = r"(\d+(?:\.\d*)?)".join(re.escape(s) for s in pattern.split("*", 1))
    best_fn = None
    best_n = None
    all_results = []
